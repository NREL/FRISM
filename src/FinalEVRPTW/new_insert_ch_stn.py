from utilities import calculate_route_cost, calculate_total_cost, is_route_feasible, reroute_infeasible_routes, flatten_route
...
def insert_charging_stations(data, routes, soc_threshold=0.6):
    updated_routes = []
    unvisited_customers = set()
    battery_capacity = data['battery_capacities'][0]
    depot = data['depot']

    all_customers = set(data['customer_nodes'])  # Global customer list from initial data
    ...
    for route in routes:
        ...
        if updated_route is None:
            ...
            unvisited_customers.update(set(node for node in route if node in data['customer_nodes']))
        else:
