from utilities import calculate_total_cost, calculate_route_cost , is_route_feasible, reroute_infeasible_routes, flatten_route
...
def plot_solution_routes(data, routes, filename=None):
    """Plots the solution routes, excluding unused vehicles."""
    coordinates = data['locations']
    colors = ['r', 'g', 'b', 'c', 'm', 'y', 'k']
    plt.figure(figsize=(10, 8))
    
    # Plot depot
    plt.scatter(coordinates[0][0], coordinates[0][1], color='red', s=100, label='Depot', marker='^')
    plt.text(coordinates[0][0], coordinates[0][1], 'Depot', fontsize=9)
    
    # Plot customers
    for idx in data['customer_nodes']:
        plt.scatter(coordinates[idx][0], coordinates[idx][1], color='blue', s=50, label='Customer' if idx == 1 else "", marker='o')
    ...
    # Plot routes for vehicles that are used (i.e., that don't stay at the depot)
    for vehicle_id, route in enumerate(routes):
        if len(route) > 2 or (len(route) == 2 and route[0] != route[1]):
            route_coords = [coordinates[node] for node in route]
            route_coords = np.array(route_coords)
            plt.plot(route_coords[:, 0], route_coords[:, 1], colors[vehicle_id % len(colors)], label=f'Vehicle {vehicle_id}')
    
    # plt.title('Routes with Charging Stations')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.legend()
    plt.grid(False) 
