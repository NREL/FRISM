...
selected_x_var_per= ["R_AGE",'HHSIZE', 'WORKER', "R_SEX_IMP",
                      'EDUC_2',"HHVEHCNT",'income_cls_1', 'income_cls_2', 'income_cls_3', 'income_cls_4',
       'income_cls_5'
                     ]
...
#############################
selected_x_var_per= ["R_AGE",'HHSIZE', 'WORKER', "R_SEX_IMP",
                      'EDUC_2',"HHVEHCNT","URBRUR","DELIV_GOOD"
                     ]
...
# %%  Grovery delivery ############################### Zero-inflated
selected_x_var_per= [ 'WORKER', 'R_AGE_C_0', 'R_AGE_C_1', 'R_AGE_C_2', 'R_AGE_C_3',
                     "URBRUR","DELIV_GOOD"]
...
# model
model=sm.ZeroInflatedPoisson(endog=Y, exog=X, exog_infl=X_inflate, inflation='logit').fit(maxiter=100)

#filename = '../Simulation/delivery_freq_model.sav'
#joblib.dump(result, filename)
model.summary()
# %%
df_good["DELIV_Groc_prd"] = model.predict(X,exog_infl=X_inflate)
plt.hist(df_good["DELIV_Groc_prd"], color ="blue", bins = Y.max())
# %%
# %%
selected_x_var_per= [ 'WORKER', 'R_AGE_C_0', 'R_AGE_C_1', 'R_AGE_C_2', 'R_AGE_C_3',
                     "URBRUR","DELIV_GOOD"]
...
#joblib.dump(result, filename)
model.summary()
df_good["DELIV_food_prd"] = model.predict(X,exog_infl=X_inflate)
plt.hist(df_good["DELIV_food_prd"], color ="blue", bins = Y.max())
# %%
...
selected_x_var_per= ['HHSIZE', 'WORKER', 'R_AGE_1', 'R_AGE_2', 'R_AGE_3',
                    'R_RACE_IMP_1', 'R_RACE_IMP_2', 'R_RACE_IMP_3',
                    'EDUC_1', 'EDUC_2', 
                    'R_RACE_IMP_0','R_RACE_IMP_1', 'R_RACE_IMP_2',
              'income_cls_1', 'income_cls_2','income_cls_3', 'income_cls_4', 'income_cls_5',
              "R_SEX_IMP"]
...
df_train=pd.DataFrame(Y)
df_train['BB_LAMBDA'] = poisson_training_results.mu
df_train['AUX_OLS_DEP'] = df_train.apply(lambda x: ((x['DELIV_GOOD_week'] - x['BB_LAMBDA'])**2 - x['BB_LAMBDA']) / x['BB_LAMBDA'], axis=1)
ols_expr = """AUX_OLS_DEP ~ BB_LAMBDA - 1"""
aux_olsr_results = smf.ols(ols_expr, df_train).fit()
...
selected_x_var_per= [ 'WORKER', 'R_AGE_0', 'R_AGE_1', 'R_AGE_2', 'R_AGE_3',
                     "URBRUR","DELIV_GOOD"]
# selected_x_var_per= ['WORKER', 'HHSIZE','R_AGE_1', 'R_AGE_2', 'R_AGE_3',
#                     'R_RACE_IMP_1', 'R_RACE_IMP_2', 'R_RACE_IMP_3',
#                     'EDUC_1', 'EDUC_2', 
#               'income_cls_1', 'income_cls_2','income_cls_3', 'income_cls_4', 'income_cls_5']
sel_x_inf= ['HHSIZE',"DELIV_GOOD",
