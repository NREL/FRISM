...
for i in range (0, df_for_qc.shape[0]):
    ...
    for s_type in ["B2B", "B2C"]:
        ...
        try: 
            df_ship_v2= pd.read_csv(f_dir+"Results_from_HPC_v2/Shipment2Fleet/"+"{}_payload_county{}_shipall.csv".format(s_type,county))
            df_for_qc.loc[i,s_type+"_ship_v2"]=df_ship_v2.shape[0]
            df_car_v2= pd.read_csv(f_dir+"Results_from_HPC_v2/Tour_plan/"+"{}_county{}_carrier_xy.csv".format(s_type,county))
...
df_for_qc.to_csv("/Users/kjeong/NREL/1_Work/1_2_SMART_2_0/Model_development/FRISM_input_output_SF/Validation/Sim_result_QC.csv")
# %%
