...
# from insert_ch_stn_6pt import flatten_route, insert_charging_stations, insert_charging_stations_single_route
from insert_ch_stn_6pt_1_13 import  insert_charging_stations, insert_charging_stations_pd,insert_charging_stations_single_route, insert_charging_stations_single_route_pd 




def variable_neighborhood_search(data, initial_routes, selected_pt, remaining_customers, max_iterations=1000, no_improvement_limit=25, num_customers=None):
    print("remaining_customers at start of VNS:", remaining_customers)
    ...
    Ensures that each remaining customer is visited exactly once.
    """
    start_time = time.time()
    current_routes = initial_routes[:]
    current_cost = calculate_total_cost(data, current_routes, selected_pt, charging_rate=data['charging_rate'])
    ...
    # Ensure remaining customers is used instead of all customers
    if remaining_customers is None:
        remaining_customers = set(data['customer_nodes'])  # Default to all customers
    else:
        remaining_customers = set(remaining_customers)
    ...
    while iteration < max_iterations and (time.time() - start_time) < max_time_seconds:
        ...
        while k < len(neighborhood_operators):
            ...
            shaken_routes = operator(data, best_routes, selected_pt, remaining_customers)
            ...
            if shaken_routes != best_routes:
                ...
                for route in shaken_routes:
                    ...
                    # print("Inserting charging stations into shaken route:", route_without_cs)
                    updated_route = insert_charging_stations_single_route(data, route_without_cs, selected_pt)
                    if updated_route is None:
                        feasible = False
                        break
                    if is_route_feasible(data, updated_route):
                        feasible_shaken_routes.append(updated_route)
                        route_customers = set(node for node in updated_route if node in remaining_customers)
                ...
                unvisited_customers = remaining_customers - combined_customers
                if feasible and not unvisited_customers:
                    shaken_cost = calculate_total_cost(data, feasible_shaken_routes, selected_pt, charging_rate=data['charging_rate'])

                    # Local Search on Shaken Solution
                    refined_routes = local_search(data, feasible_shaken_routes, selected_pt, remaining_customers)
...
def local_search(data, routes, selected_pt, remaining_customers):
    """
    Performs local search on the given EV routes to improve the solution.
    Evaluates based on total route costs.
    Ensures that each remaining customer is visited exactly once.
    """
    ...
    improved = True
    remaining_customers = set(remaining_customers)  # Convert to set for quick checks
    ...
    while improved:
        ...
        for route_index, route in enumerate(best_routes):
            ...
            route_without_cs = [node for node in route if node not in data['charging_stations']]
            new_route = intra_route_2opt_swap_single_route(data, route_without_cs, selected_pt, remaining_customers)
            if new_route and new_route != route_without_cs:
                # Insert charging stations
                updated_route = insert_charging_stations_single_route(data, new_route, selected_pt)
                if updated_route is None:
                ...
                if is_route_feasible(data, updated_route):
                    ...
                    new_cost = calculate_total_cost(data, new_routes, selected_pt, charging_rate=data['charging_rate'])
...
def intra_route_2opt_swap_single_route(data, route, selected_pt, remaining_customers):
    """
    Applies 2-opt swap on a single route without charging stations, considering only remaining customers.
    """
    if len(route) <= 3:
    ...
    # Identify remaining customer node indices
    remaining_customers = set(remaining_customers)
    customer_indices = [i for i, node in enumerate(route) if node in remaining_customers]
    ...
    for i in range(len(customer_indices) - 1):
        for j in range(i + 1, len(customer_indices)):
            ...
            # Check feasibility
            if is_route_feasible(data, updated_route):
                new_cost = calculate_route_cost(data, updated_route, selected_pt,charging_rate=data['charging_rate'])
                if new_cost < best_cost:
...
def intra_route_2opt_swap(data, routes, selected_pt, remaining_customers):
    """
    ...
    for route_index, route in enumerate(routes):
        ...
        for i in range(len(customer_indices) - 1):
            for j in range(i + 1, len(customer_indices)):
                ...
                # Check feasibility
                # print("Attempting 2-opt swap on route:", new_route)
                route_with_charging, unvisited_nodes = insert_charging_stations(data, [new_route],  remaining_customers, selected_pt)
                if unvisited_nodes:
                    continue  # Skip infeasible route
                route_with_charging = route_with_charging[0]

                if is_route_feasible(data, route_with_charging):
                    new_cost = calculate_route_cost(data, route_with_charging, selected_pt,  data['charging_rate'])
                    if new_cost < best_cost:
...
def intra_route_or_opt(data, routes,selected_pt, remaining_customers):
    """
    Applies the Or-opt move within each route.
    Ensures that only `remaining_customers` are considered and that no customers are lost or duplicated.
    """
    ...
    for route_index, route in enumerate(routes):
        ...
        # Restrict to remaining customers
        original_customers = set(node for node in route_without_cs if node in remaining_customers)
        ...
        for segment_length in range(1, 4):
            for i in range(1, len(route_without_cs) - segment_length):
                ...
                for j in range(1, len(remaining_route)):
                    ...
                    # Check that the new route contains the same `remaining_customers`
                    new_customers = set(node for node in new_route if node in remaining_customers)
    ...
    # After updating all routes, check for missing or duplicated remaining customers
    ...
    if missing_customers:
        print(f"Warning: Missing customers in new_routes after intra-route Or-opt: {missing_customers}")
        # Handle missing customers appropriately
    if duplicated_customers:
        print(f"Warning: Duplicated customers in new_routes after intra-route Or-opt: {duplicated_customers}")
...
def inter_route_swap(data, routes, selected_pt, remaining_customers):
    """
    Swaps customer nodes between two different routes, focusing on `remaining_customers`.
    ...
    customers1 = [node for node in route1_without_cs if node in remaining_customers]
    customers2 = [node for node in route2_without_cs if node in remaining_customers]

    if not customers1 or not customers2:
        return routes  # Cannot swap if no `remaining_customers` in one of the routes
    ...
    customers_route1 = set(node for node in route1_new if node in remaining_customers)
    customers_route2 = set(node for node in route2_new if node in remaining_customers)
    duplicates = customers_route1 & customers_route2
    if duplicates:
        print(f"Warning: Duplicate customers detected after swap: {duplicates}")
    ...
    # Insert charging stations using the helper function
    updated_route1 = insert_charging_stations_single_route(data, route1_new, selected_pt)
    updated_route2 = insert_charging_stations_single_route(data, route2_new, selected_pt)
    ...
    if is_route_feasible(data, updated_route1) and is_route_feasible(data, updated_route2):
        ...
        if missing_customers:
            print(f"Warning: Missing customers after inter-route swap: {missing_customers}")
            return routes  # Revert to original routes if critical customers are missing
        if duplicated_customers:
            print(f"Warning: Duplicated customers after inter-route swap: {duplicated_customers}")
            return routes  # Revert to original routes if duplicates are found

        # Calculate total cost
        total_cost = calculate_total_cost(data, new_routes,selected_pt, data['charging_rate'])
...
def inter_route_relocate(data, routes, selected_pt, remaining_customers):
    """
    Moves a customer node from one EV route to another, focusing on `remaining_customers`.
    Ensures that each `remaining_customer` is visited exactly once.
    """
    if len(routes) < 2:
    ...
    source_customers = [node for node in source_route_without_cs if node in remaining_customers]
    ...
    # Check for duplicates
    customers_source = set(node for node in source_route_new if node in remaining_customers)
    customers_target = set(node for node in target_route_new if node in remaining_customers)
    duplicates = customers_source & customers_target
    if duplicates:
        print(f"Warning: Duplicate customers detected after relocation: {duplicates}")
        return routes  # Revert to original routes
    ...
    # Insert charging stations using the helper function
    updated_source_route = insert_charging_stations_single_route(data, source_route_new, selected_pt)
    updated_target_route = insert_charging_stations_single_route(data, target_route_new, selected_pt)
    ...
    if is_route_feasible(data, updated_source_route) and is_route_feasible(data, updated_target_route):
        ...
        if missing_customers:
            print(f"Warning: Missing customers after inter-route relocate: {missing_customers}")
            return routes  # Revert to original routes if critical customers are missing
        if duplicated_customers:
            print(f"Warning: Duplicated customers after inter-route relocate: {duplicated_customers}")
            return routes  # Revert to original routes if duplicates are found
...
def route_merge(data, routes, selected_pt, remaining_customers):
    ...
    # Insert charging stations using the helper function
    updated_merged_route = insert_charging_stations_single_route(data, merged_route, selected_pt)
