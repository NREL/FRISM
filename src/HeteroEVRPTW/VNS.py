...
from insert_ch_stn import flatten_route, insert_charging_stations, insert_charging_stations_single_route




def variable_neighborhood_search(data, initial_routes, max_iterations=1000, no_improvement_limit=25, num_customers=None, remaining_customers=None):
    ...
    while iteration < max_iterations and (time.time() - start_time) < max_time_seconds:
        ...
        while k < len(neighborhood_operators):
            ...
            shaken_routes = operator(data, best_routes, remaining_customers)
            ...
            if shaken_routes != best_routes:
                ...
                if feasible and not unvisited_customers:
                    ...
                    refined_routes = local_search(data, feasible_shaken_routes, remaining_customers)
...
def local_search(data, routes, remaining_customers):
    """
    ...
    while improved:
        ...
        for route_index, route in enumerate(best_routes):
            ...
            route_without_cs = [node for node in route if node not in data['charging_stations']]
            new_route = intra_route_2opt_swap_single_route(data, route_without_cs, remaining_customers)
...
def intra_route_2opt_swap_single_route(data, route, remaining_customers):
    """
...
def intra_route_2opt_swap(data, routes, remaining_customers):
    """
    ...
    for route_index, route in enumerate(routes):
        ...
        for i in range(len(customer_indices) - 1):
            for j in range(i + 1, len(customer_indices)):
                ...
                # Check feasibility
                route_with_charging, unvisited_nodes = insert_charging_stations(data, [new_route], remaining_customers)
                if unvisited_nodes:
                    continue  # Skip infeasible route
...
def intra_route_or_opt(data, routes, remaining_customers):
    """
...
def inter_route_swap(data, routes, remaining_customers):
    """
...
def inter_route_relocate(data, routes, remaining_customers):
    """
...
def route_merge(data, routes, remaining_customers):
    """
