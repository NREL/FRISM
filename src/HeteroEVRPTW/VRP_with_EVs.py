...
def print_solution(data, manager, routing, solution, tour_df, carr_id, carrier_df, payload_df, prob_type,
    ...
    for vehicle_id in range(data['num_vehicles']):
        ...
        if start_loc != next_loc:
            ...
            # print("#############\n")
            # print(c_prob)
            # true_depot = c_prob['depot_zone'].values[0]
...
def form_solve(data, tour_df, carr_id, carrier_df, payload_df, prob_type, count_num, ship_type, c_prob,
    ...
    # try:
        ...
        # calling Megh's print solution
        ev_routes, ice_routes = print_ICE_EV_solts(data, solution, routing, manager)
        # print('\n')
        return ev_routes, ice_routes
    # except Exception as e:
...
def print_ICE_EV_solts(data, solution, routing, manager):
    ...
    for vehicle_id in range(data['num_vehicles']):
        ...
        elif 'Diesel' in vehicle_type:
            ice_routes.append(route)
        else: print('did not find veh type: ', vehicle_type)
