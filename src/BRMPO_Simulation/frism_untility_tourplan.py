...
def create_data_model(df_prob, depot_loc, prob_type, v_df, f_prob, c_prob, carrier_id,
                      tt_df, dist_df, veh, commodity, ship_index, path_stops, dic_taz):
        ...
        for i in data['loc_zones']:
            for j in data['loc_zones']:
                temp =pd.DataFrame({
                    'origin' : i,
                    'destination': j,
                        'TIME_minutes': tt_df[dic_taz[i]][dic_taz[j]]
                }, index=[n]
                )
                n=n+1
        ...
        for i in data['loc_zones']:
            for j in data['loc_zones']:
                temp =pd.DataFrame({
                    'Origin' : i,
                    'Destination': j,
                        'dist': dist_df[dic_taz[i]][dic_taz[j]]
                }, index=[n]
                )
                n=n+1
...
def input_files_processing(travel_file, carrier_file, payload_file, vehicleType_file):
    ...
    try:
        ...
        tt_df = f['data']['htrk_time'] # minutes
        dist_df = f['data']['htrk_dist']  # Slow step
        dic_taz={}
        ...
        return tt_df, dist_df, c_df, p_df, v_df, vc_df, dic_taz
    except Exception as e:
        prefix = ''
