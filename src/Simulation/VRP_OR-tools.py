    ...
    for carr_id in c_df['carrier_id'].unique():
        try:
            ...
            if len(df_prob)> 0 and len(f_prob)> 0 and len(c_prob)> 0 and len(vc_prob)>0:
                print('Solvign problem for carrier is ', carr_id)
                ...
                if len(df_prop) == 0:
                    print('Skipping carrier, number of payload is 0')
                if len(f_prob) == 0 or len(vc_prob) == 0:
                    print('Skipping carrier, number of vehicle type is 0')
                if len(c_prob) == 0:
                    print('Skipping carrier, carrier dataframe size is 0')

        except:
            print('Could not solve problem for carrier: ', carr_id)
            print('\n')
