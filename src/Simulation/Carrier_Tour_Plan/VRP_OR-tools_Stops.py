...
def create_data_model(df_prob, depot_loc, prob_type, v_df, f_prob, c_prob, carrier_id,
                     md_start_id, hd_start_id, CBGzone_df, tt_df, dist_df, veh, commodity, ship_index):
    ...
    if veh == 'md':
        ...
        for i in range(0, int(f_prob['hd_veh'].values[0])):
            ...
            # Adding max and slack stops per vehicle
            if commodity != 2 and ship_index =='internal':
                prob = random.uniform(0, 1)
                temp = stop_df[stop_df.Cum_Prob >= prob].reset_index()
                # print(temp)
        ...
        for comm in p_df[p_df['carrier_id']==carr_id]['commodity'].unique():
            for index in p_df[(p_df['carrier_id']==carr_id) & (p_df['commodity']==comm)]['ship_index'].unique():
                for veh in veh_types:
                    if len(used_veh) > 0:
                        print('before: ', f_prob)
                        f_prob.loc[0,veh] = f_prob.loc[0,veh+'_veh']-len(used_veh)
                        print('after: ', f_prob)
                    ...
                    # if len(df_prob)> 0 and len(f_prob)> 0 and len(c_prob)> 0 and len(vc_prob)>0:
                        ...
                        data = create_data_model(df_prob, depot_loc, prob_type, v_df, f_prob, c_prob, carr_id,
                                                md_start_id, hd_start_id, CBGzone_df, tt_df, dist_df, veh, comm, index)
                        ...
                        if index == 'internal' and comm != 2:
                            ...
                            for veh in range(data['num_vehicles']):
                                stop_dimension.SetCumulVarSoftUpperBound(routing.End(veh), data['vehicle_max_stops'][veh], penalty_stop)
