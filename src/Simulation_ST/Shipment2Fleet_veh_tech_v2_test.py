...
def main(args=None):
    ...
    truckings.to_csv(fdir_in_out+'/Sim_outputs/temp_save/truckings.csv', index = False, header=True)

    truckings=pd.read_csv(fdir_in_out+'/Sim_outputs/temp_save/truckings.csv')
    ...
    # 0516 added 
    df_hh_D_GrID=pd.read_csv(fdir_in_out+'/Sim_outputs/Generation/B2C_daily_%s.csv' %sel_county) 
    # 0516 end 
    ...
    df_hh_D_GrID_only=df_hh_D_GrID[df_hh_D_GrID['assigned_carrier'] !="no"].reset_index(drop=True)
        ...
        df_hh_D_GrID.to_csv(fdir_in_out+'/Sim_outputs/temp_save/df_hh_D_GrID_carrier_assigned_county%s_%s.csv' %(sel_county, "all"), index = False, header=True)
        # vehicle type assign
        ...
        if not file_exists(config.fdir_main_output + str(2018)+"/"):
            os.makedirs(config.fdir_main_output + str(2018)+"/")
        dir_out= config.fdir_main_output + str(2018)+"/"   
        payloads.to_csv (dir_out+config.fnm_B2C_payload+"_county{}_ship{}_s{}_y{}_sr{}.csv".format(sel_county, ship_direction,"Base", 2018,sample_ratio), index = False, header=True)
        carriers.to_csv (dir_out+config.fnm_B2C_carrier+"_county{}_ship{}_s{}_y{}_sr{}.csv".format(sel_county, ship_direction,"Base", 2018,sample_ratio), index = False, header=True)
