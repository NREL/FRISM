...
import time
import json
from frism_utility_distributionchannel import b2c_forhire_distribution_channel, b2c_create_output, veh_type_create, b2c_ondemand_distribution_channel
...
def main(args=None):
    ...
    print ("Creating B2C shipments")
    b2c_shipment_file_path = input_variables["frism_data_folder"]+input_variables['sub_folder_b2c_demand']+"{0}_{1}/".format(year, scenario)
    daily_shipment = B2C_DailyShipment(b2c_file_path= b2c_shipment_file_path,
                                        zone_df= network.zone,
                                        hh_aggregation_num= input_variables["b2c_hosehold_aggregation"], 
                                        delivery_factor=input_variables["b2c_day_factor"],
    ...
    if b2c_type in ["all", "goods"]:     
        print ("B2C for-hire processing")
        selected_leasings = firms.leasings_agg[firms.leasings_agg["st"]==state_abbr].reset_index(drop=True)
        b2c_shipment= daily_shipment.df_hh_d_good_sample

        final_b2c_forhire, selected_carrier, selected_leasings_b2c= b2c_forhire_distribution_channel (carriers.carrier,
                                                                                                    b2c_shipment,
                                                                                                    network.zone, 
                                                                                                    vius.vius, 
                                                                                                    network.od_distance, 
                                                                                                    vehicle_energy.dic_energy, 
                                                                                                    selected_leasings, 
                                                                                                    input_variables, 
                                                                                                    dic_veh,
                                                                                                    mdhdv_adjustment)
        print ("Processing B2C shipment to fleet format")
        carrier_out, payloads_out = b2c_create_output(final_b2c_forhire,selected_carrier,departure_distribution.departure_distribution,input_variables, ship_type, dic_veh, network.local_zone)
    ...
    if b2c_type in ["all", "ondemand"]:
        print ("Processing and wirting B2C ondemand shipment to payload format")  
        goods_type="food"
        payloads_out=b2c_ondemand_distribution_channel(daily_shipment.df_hh_d_food_sample.loc, network.ondemand_loc, network.od_distance, network.zone, sel_county, goods_type)
