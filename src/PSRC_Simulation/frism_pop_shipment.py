...
def countyid_from_cbgid (state_id, cbgid):
    cbgid_str = str(int(cbgid))
    if (len(cbgid_str)==12 and cbgid_str[0:2]==str(state_id).zfill(2)):
        return int(cbgid_str[2:5])
    elif (len(cbgid_str)==11 and cbgid_str[0:1]==str(state_id).zfill(1)):
...
class Network:
    ...
    def _load_zone_file(self, file_path):
        ...
        self.zone["County"]=self.zone["GEOID"].apply(lambda x: countyid_from_cbgid (self.state_id, x))
    ...
    def _local_external_zone_file(self, file_path):
        ...
        for _, row in in_zones.iterrows():
            self.dic_exteranl_coordinate[int(row.i)] = (row.geometry.x, row.geometry.y)
...
class Carriers:
    ...
    * A class for the carier: Generate B2B and B2C seperately *
    ...
    def _load_carrier_file(self, file_path, shipment_type):
        ...
        warehouses["County"]=warehouses["MESOZONE"].apply(lambda x: countyid_from_cbgid (self.state_id, x))
    ...
    def _process_domestic_flow(self, file_path):
        ...
        for filename in list_file:
            ...
            if temp.shape[0] >0:
                ...
                temp["SellerCounty"]=temp["SellerZone"].apply(lambda x: countyid_from_cbgid (self.state_id, x))
                temp["BuyerCounty"]=temp["BuyerZone"].apply(lambda x: countyid_from_cbgid (self.state_id, x))
                if self.sel_county:
