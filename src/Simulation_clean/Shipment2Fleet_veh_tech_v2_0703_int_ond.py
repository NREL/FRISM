...
# %%    
######################### General CODES ############################
def genral_input_files_processing(firm_file, port_file, warehouse_file, leasing_file, stock_file, target_year, scenario, dist_file,CBG_file, ship_type,list_error_zone, county_list):
    global dic_energy
    ...
    if ship_type == 'B2B':
        ...
        firm_file_xy=fdir_firms+port_file
        ports=gpd.read_file(firm_file_xy)
        for i in range(0,ports.shape[0]):
            temp_line=pd.DataFrame(data={"CBPZONE": [ports.loc[i,"CBPZONE"]],                        
                    "FAFZONE": [ports.loc[i,"FAF"]],                       
                    "Industry_NAICS6_Make": ["NA"],           
                    "Commodity_SCTG": [0],                 
                    "Emp": [0],                            
                    "SellerID": [ports.loc[i,"PORTID"]],                          
                    "MESOZONE":[ports.loc[i,"MESOZONE"]],                       
                    "y":[ports.loc[i,"lat"]],                            
                    "x": [ports.loc[i,"lon"]],                            
                    "st": [0],                             
                    "stname": ["NA"],                         
                    "EV_powertrain (if any)": ["NA"],         
                    "fleet_id": [0],                       
                    "Diesel Class 4-6 Vocational": [0] ,    
                    "Diesel Class 7&8 Tractor": [0],       
                    "Diesel Class 7&8 Vocational": [0],    
                    "Electric Class 4-6 Vocational": [0],  
                    "Electric Class 7&8 Vocational": [0],  
                    "Gasoline Class 4-6 Vocational": [0],  
                    "n_trucks": [0],                       
                    "Electric Class 7&8 Tractor": [0]})
            firms=pd.concat([firms,temp_line],ignore_index=True)
    ...
    if ship_type == 'B2C':
        ...
        for veh_type in input_veh_list[:-1]:
            truckings[veh_type]=truckings[veh_type].apply(lambda x: x*2)
        truckings['md_capacity']=truckings.apply(lambda x: (x['Diesel Class 4-6 Vocational'] + x['Electric Class 4-6 Vocational'])*md_max_load, axis=1)
        truckings['hdt_capacity']=truckings.apply(lambda x: (x['Diesel Class 7&8 Tractor'] + x['Electric Class 7&8 Tractor']) *hd_max_load, axis=1)
        truckings['hdv_capacity']=truckings.apply(lambda x: (x['Diesel Class 7&8 Vocational'] + x['Electric Class 7&8 Vocational']) *hd_max_load, axis=1)
    ...
    if ship_type == 'B2C':
        temp_df=pd.read_csv(fdir_truck+'delivery_fleet_departure.csv', header=0, sep=',')
        ...
        df_dpt_dist=pd.DataFrame()
        for g in ["MD","HD"]:
            temp_df=pd.read_csv(fdir_truck+'depature_dist_by_cbg_{}.csv'.format(g), header=0, sep=',')
            temp_df=temp_df.groupby(['start_hour'])['Trip'].sum().reset_index(name='obs_trips')
            temp_df["veh_type"]=g
            temp_df['pro_trips']= temp_df['obs_trips']/temp_df['obs_trips'].sum()
            temp_df['cdf_low']=0.0
            temp_df['cdf_up']=temp_df['pro_trips']
            for i in range(1,temp_df.shape[0]):
                temp_df.loc[i,'cdf_up']=temp_df.loc[i-1,'cdf_up']+temp_df.loc[i,'pro_trips']
                temp_df.loc[i,'cdf_low']=temp_df.loc[i-1,'cdf_up'] 
            df_dpt_dist=pd.concat([df_dpt_dist,temp_df], ignore_index=True).reset_index(drop=True) 
        temp_df=df_dpt_dist
        temp_df=temp_df.groupby(['start_hour'])['obs_trips'].sum().reset_index(name='obs_trips')
        temp_df["veh_type"]="all"
        temp_df['pro_trips']= temp_df['obs_trips']/temp_df['obs_trips'].sum()
        temp_df['cdf_low']=0.0
        temp_df['cdf_up']=temp_df['pro_trips']
        for i in range(1,temp_df.shape[0]):
...
def depot_time_depart_v2(df_dpt_dist,ship_type, veh_type):
    if ship_type == 'B2C': 
        ...
        if sel_time>=10 and sel_time <16: 
            d_time =random.randrange(sel_time*60, (sel_time+1)*60, 10)- random.randrange(1.5*60, 5*60, 10)
        elif sel_time>=16 and sel_time <20:
            d_time =random.randrange(sel_time*60, (sel_time+1)*60, 10)- random.randrange(2*60, 10*60, 10)
        elif sel_time>=20:
            d_time =random.randrange(sel_time*60, (sel_time+1)*60, 10)- random.randrange(6*60, 23*60, 10)         
        else:     
            d_time =random.randrange(sel_time*60, (sel_time+1)*60, 10) 
    else:
        print ("Please define shipment type: B2B or B2C")        
...
def b2c_input_files_processing(CBGzone_df, possilbe_delivey_days, sel_county, list_error_zone,growth_factor, year):
    ...
    df_per_final = pd.read_csv(fdir_in_out+'/Sim_outputs/Generation/per_del_{}.csv'.format(str(year)), header=0, sep=',')
    ...
    df_per_final[['D_selection_goods', 'D_num_goods']] = df_per_final['DELIV_GOOD'].apply(lambda x: b2c_good_select(x, possilbe_delivey_days,growth_factor, "goods")).to_list()
    df_per_final[['D_selection_groc', 'D_num_groc']] = df_per_final['DELIV_GROC'].apply(lambda x: b2c_good_select(x, possilbe_delivey_days,growth_factor, "groc")).to_list()
    df_per_final[['D_selection_food', 'D_num_food']] = df_per_final['DELIV_FOOD'].apply(lambda x: b2c_good_select(x, possilbe_delivey_days,growth_factor, "food")).to_list()
    ...
    if df_per_goods.shape[0] >0:
        df_hh_goods = df_per_goods.groupby(["household_id",'block_id','GEOID',"County",'MESOZONE'])["D_num_goods"].agg(hh_num_goods='sum').reset_index()
        ...
        df_hh_goods_delivery = pd.DataFrame(columns = ["household_id","block_id","GEOID","County",'MESOZONE',
                                           "hh_num_goods","hh_del_goods","shipment_id","D_truckload"])              
    if df_per_groc.shape[0] >0:    
        df_hh_groc = df_per_groc.groupby(["household_id",'block_id','GEOID',"County",'MESOZONE'])["D_num_groc"].agg(hh_num_groc='sum').reset_index()
        ...
        df_hh_groc_delivery = pd.DataFrame(columns = ["household_id","block_id","GEOID","County",'MESOZONE',
                                           "hh_num_goods","hh_del_goods","shipment_id","D_truckload"])  
    if df_per_groc.shape[0] >0: 
        df_hh_food = df_per_food.groupby(["household_id",'block_id','GEOID',"County",'MESOZONE'])["D_num_food"].agg(hh_num_food='sum').reset_index()
        ...
        df_hh_food_delivery = pd.DataFrame(columns = ["household_id","block_id","GEOID","County",'MESOZONE',
                                           "hh_num_goods","hh_del_goods","shipment_id","D_truckload"])  
    return df_hh_goods_delivery, df_hh_groc_delivery, df_hh_food_delivery
...
def on_demnad_output (df_input_org,CBGzone_df, loc_group, loc_df,tt_df,goods_type ):
    
    df_input=df_input_org.groupby(['GEOID','MESOZONE'])["household_id"].agg(n_households='count').reset_index()
...
def b2c_create_output(df_del,truckings,df_dpt_dist, ship_type):
    ...
    'pu_y',
    'ship_index',
    'true_pu_zone', # 0610 add true zone
    'true_del_zone', # 0610 add true zone
    'truck_mode', # 0610 add truck mode
    'BuyerNAICS',# 0610 add 
    'SellerNAICS'])# 0610 add
    ...
    payloads['true_del_zone']=df_del['MESOZONE']
    ...
    payloads['truck_mode']="For-hire Truck"
    payloads['BuyerNAICS']=492000
    payloads['SellerNAICS']=492000 
    ...
    'depot_upper',
    'depot_time_before',
    'depot_time_after','c_x','c_y','true_depot_zone'] + output_veh_list +["ev_type"])
    ...
    carriers['true_depot_zone']=carrier_input['MESOZONE']
    carriers['contract_firms']='Nan'
    carriers['depot_lower']= carriers['depot_zone'].apply(lambda x: depot_time_depart_v2(df_dpt_dist,ship_type, "MD"))
...
def b2b_input_files_processing(firms, leasings, truckings, dist_df, CBGzone_df, sel_county, ship_direction, commodity_list, weight_theshold, list_error_zone, county_list, df_vius, b2b_day_factor,year, scenario,daily_demand_creator):
    ...
    FH_B2B= B2BF_T_D[(B2BF_T_D['mode_choice']=="For-hire Truck") & (B2BF_T_D['ship_category'].isin(["domestic"]))].reset_index(drop=True)
    FH_B2B_INT= B2BF_T_D[(B2BF_T_D['mode_choice']=="For-hire Truck") & (B2BF_T_D['ship_category'].isin(["import", "export"]))].reset_index(drop=True)
    ...
    for seller_id in PV_B2B['SellerID'].unique():
            ...
            for j in range(0, temp.shape[0]):
                ...
                if  D_num_firm+E_num_firm ==0:
                    if D_num_leasing+E_num_leasing ==0:
                        D_num_leasing=leasings_orig[(leasings_orig["powertrain"]=="Diesel")][v_type].sum()
                        E_num_leasing=leasings_orig[(leasings_orig["powertrain"]==ev_class)][v_type].sum()
        ...
        PV_B2B_ship=pd.concat([PV_B2B_ship,PV_sel_ship], ignore_index=True).reset_index(drop=True)
        # update stock
        firm_index= firms[firms['SellerID'] == seller_id].index.item()
        for key in dic_leased.keys():
            add_to_firm =dic_leased[key]    
            if key ==  "md_D_num":
    ...
    #for seller_id in [919143, 908126, 907957, 906687]:
        ...
        FH_sel.loc[:,'assigned_carrier']="no"
        FH_sel.loc[:,'veh_type_agg']="U"
        for i in range(0,FH_sel.shape[0]):
            ...
            if sel_busID != "no":
                temp['assigned_carrier'].iloc[0]=sel_busID
                temp['veh_type_agg'].iloc[0]=v_type
            ...
            temp=temp[temp['assigned_carrier'] !="no"].reset_index(drop=True)

            FH_sel_ship=pd.DataFrame()
            for i in range(0,temp.shape[0]):
                v_type= temp['veh_type_agg'].iloc[i]
                carr_id= temp['assigned_carrier'].iloc[i]
                ...
                if v_type=="md":
                    ...
                    max_load= hd_max_load    
                load=temp["D_truckload"].iloc[i]
                num_shipment=int(load/max_load)+1
                temp_2=pd.concat([temp.iloc[[i]]]*num_shipment, ignore_index=True)
                if num_shipment ==1:
                    temp_2["D_truckload"]= load
                else:    
                    temp_2["D_truckload"]=max_load
                    temp_2["D_truckload"].iloc[-1]=load-max_load*(num_shipment-1)
                temp_2["payload_rate"]= temp_2["D_truckload"].apply(lambda x: x/max_load).reset_index(drop=True) # 1:full 0:partial
                temp_2["veh_type_agg"]= v_type
                temp_2["veh_type"] = "U" # unassigned
                for j in range(0, temp_2.shape[0]):
                    D_num_firm = dic_firm_stock[v_type+"_D_num"]
                    E_num_firm = dic_firm_stock[v_type+"_E_num"]
                    if D_num_firm -temp_2['payload_rate'].iloc[j] <0:
                        D_num_firm =0.1 
                    if E_num_firm -temp_2['payload_rate'].iloc[j] <0:
                        E_num_firm =0 
                    if D_num_firm+E_num_firm -temp_2['payload_rate'].iloc[j] <0:
                        D_num_firm=1
                        E_num_firm=1                 
                    powerT=util_powertrain(D_num_firm, E_num_firm, dic_energy, temp_2['Distance'].iloc[j], dic_veh[v_type], ev_class, 0.5)
                    dic_firm_stock[v_type+"_"+powerT+"_num"] = dic_firm_stock[v_type+"_"+powerT+"_num"]-temp_2['payload_rate'].iloc[j]
                    if powerT == "D":
                        p_lable="Diesel"
                    else:
                        p_lable=ev_class          
                    temp_2["veh_type"].iloc[j]=v_type+"_"+powerT+"_"+p_lable
                ...
                FH_sel_ship= pd.concat([FH_sel_ship,temp_2], ignore_index=True).reset_index(drop=True)
                del temp_2              
    ...
    # int 
    for seller_id in FH_B2B_INT['SellerID'].unique():
    #for seller_id in [919143, 908126, 907957, 906687]:
        # seller_id= 98420   
        FH_sel = FH_B2B_INT[FH_B2B_INT['SellerID'] == seller_id].reset_index()
        FH_sel["Dist_group"]= FH_sel["Distance"].apply(lambda x: dist_group(x))
        FH_sel.loc[:,'assigned_carrier']="no"
        FH_sel.loc[:,'veh_type_agg']="U"
        for i in range(0,FH_sel.shape[0]):
...
def b2b_d_shipment_by_commodity(fdir, weight_theshold, CBGzone_df,sel_county,ship_direction, county_wo_sel, b2b_day_factor,year, scenario): 
    ...
    for filename in glob.glob(fdir+sub_fdir+'*.csv'):
        temp= pd.read_csv(filename, header=0, sep=',',index_col=0)
        if temp.shape[0] >0:
            ...
            temp = temp.drop("MESOZONE", axis=1)
    ...
    temp_import=temp.query('D_selection ==1')
    if sample_ratio <100: 
        temp_import=sampling_int_ship(temp_import, sample_ratio, bin_size) 
    ...
    for i in range(0,temp_import.shape[0]):
        temp_line=pd.DataFrame(data= {'BuyerID': [temp_import.loc[i,"BuyerID"]] ,
                ...
                "SellerNAICS":[temp_import.loc[i,"TYPE"]],
                "TruckLoad":[temp_import.loc[i,"TruckLoad"]],
                "SCTG_Group":[temp_import.loc[i,"SCTG_Group"]],
                "Commodity_SCTG": [temp_import.loc[i,"Commodity_SCTG"]],
                ...
                "Travel_time":[0],
                "SellerCounty":[temp_import.loc[i,"SellerCounty"]],
                "BuyerCounty":[temp_import.loc[i,"BuyerCounty"]],
                "D_truckload":[temp_import.loc[i,"D_truckload"]],
                "D_selection":[temp_import.loc[i,"D_selection"]],
    ...
    # export
    filename = fdir+sub_fdir+'int_flow/export_OD_with_seller.csv'
    temp= pd.read_csv(filename, header=0, sep=',')
    temp["SellerID"]=temp["SellerID"].astype("int")
    temp["SellerID"]=temp["SellerID"].apply(lambda x: str(x)+ "_1")
    ...
    for i in range(0,temp_import.shape[0]):
        temp_line=pd.DataFrame(data= {'BuyerID': [temp_import.loc[i,"BuyerID"]] ,
                "BuyerZone": [temp_import.loc[i,"BuyerZone"]],
                "BuyerNAICS":[temp_import.loc[i,"TYPE"]],
                "SellerID":[temp_import.loc[i,"SellerID"]],
                "SellerZone":[temp_import.loc[i,"SellerZone"]],
                "SellerNAICS":[temp_import.loc[i,"SellerNAICS"]],
                "TruckLoad":[temp_import.loc[i,"TruckLoad"]],
                "SCTG_Group":[temp_import.loc[i,"SCTG_Group"]],
                "Commodity_SCTG": [temp_import.loc[i,"Commodity_SCTG"]],
                "shipment_id":[temp_import.loc[i,"bundle_id"]],
                "orig_FAFID":[temp_import.loc[i,"dms_orig"]],
                "dest_FAFID":[temp_import.loc[i,"FAF"]],
                "mode_choice":[temp_import.loc[i,"mode_choice"]],
                "probability":[1],
                "Distance":[temp_import.loc[i,"Distance"]],
                "Travel_time":[0],
...
def b2b_veh_type_truckload_prior(SCTG_Group,Distance, D_truckload, df_vius,dic_firm_stock, dic_leasing_stock):
    ...
    md_leasing_num = dic_leasing_stock["md_D_num"]  + dic_leasing_stock["md_E_num"]
    hdt_leasing_num= dic_leasing_stock["hdt_D_num"] + dic_leasing_stock["hdt_E_num"]
    hdv_leasing_num= dic_leasing_stock["hdv_D_num"] + dic_leasing_stock["hdv_E_num"]

    if md_num+hdt_num+hdv_num==0:
        md_prior=md_leasing_num/(md_leasing_num+hdt_leasing_num+hdv_leasing_num)
        hdt_prior=hdt_leasing_num/(md_leasing_num+hdt_leasing_num+hdv_leasing_num)
        hdv_prior=hdv_leasing_num/(md_leasing_num+hdt_leasing_num+hdv_leasing_num)  
    else:
...
def b2b_create_output(B2BF_PV,B2BF_FH,truckings,df_dpt_dist, ship_type, ex_zone_list, firms,ex_zone):
    ...
    'depot_time_before',
    'depot_time_after',
    'c_x','c_y','true_depot_zone']+ output_veh_list +["ev_type"]) # 0610 add true zone
    ...
    carriers['true_depot_zone']=PV_T_D['SellerZone'].apply(lambda x: x) # 0610 add true zone
    carriers['contract_firms']=PV_T_D['Bus_ID'].apply(lambda x: [x])
    carriers['depot_lower']= PV_T_D.apply(lambda x: depot_time_depart_v2(df_dpt_dist,ship_type, "all"), axis=1)
    carriers['depot_upper']= carriers['depot_lower'].apply(lambda x: 24*60 if x < 19*60 else x + 5*60)#carriers['depot_lower'].apply(depot_time_close)
    ...
    for i in range(0,len(output_veh_list)):
        carriers[output_veh_list[i]]=PV_T_D[input_veh_list[i]].apply(lambda x: x+25 if x>0 else x)
    ...
    temp['true_depot_zone']=temp_FH_T_D['MESOZONE'].apply(lambda x: x) # 0610 add true zone
    temp['contract_firms']=temp_FH_T_D['contract_firms']
    temp['depot_lower']= temp_FH_T_D.apply(lambda x: depot_time_depart_v2(df_dpt_dist,ship_type, "all"), axis=1)
    temp['depot_upper']= temp['depot_lower'].apply(lambda x: 24*60 if x < 19*60 else x + 5*60)
    ...
    for i in range(0,len(output_veh_list)):
        temp[output_veh_list[i]]=temp_FH_T_D[input_veh_list[i]].apply(lambda x: x+25 if x>0 else x)
    ...
    payloads['true_del_zone']=B2BF_PV['BuyerZone'] # 0610 add true zone
    load_min =B2BF_PV.D_truckload.min()
    load_max = B2BF_PV.D_truckload.max()
    payloads['del_stop_duration']=B2BF_PV.apply(lambda x: int(np.random.gamma(2, 1, 1)[0]*((x['D_truckload'] -load_min)/(load_max-load_min))*60), axis=1)
    ...
    payloads['truck_mode']=B2BF_PV['mode_choice']
    payloads['BuyerNAICS']=B2BF_PV['BuyerNAICS']
    payloads['SellerNAICS']=B2BF_PV['SellerNAICS']
    ...
    payloads_FH['true_pu_zone']=B2BF_FH['SellerZone'] # 0610 add true zone
    payloads_FH['true_del_zone']=B2BF_FH['BuyerZone'] # 0610 add true zone
    load_min =B2BF_FH.D_truckload.min()
    load_max = B2BF_FH.D_truckload.max()
    payloads_FH['pu_stop_duration']= B2BF_FH.apply(lambda x: int(np.random.gamma(2, 1, 1)[0]*((x['D_truckload'] -load_min)/(load_max-load_min))*60), axis=1)
    ...
    payloads_FH['del_stop_duration']=B2BF_FH.apply(lambda x: int(np.random.gamma(2, 1, 1)[0]*((x['D_truckload'] -load_min)/(load_max-load_min))*60), axis=1)
    ...
    payloads_FH['pu_tw_upper']=B2BF_FH['inbound_index'].apply(lambda x: 19*60 if x==1 else 22*60)
    payloads_FH['del_tw_lower']=payloads_FH.apply(lambda x: x['pu_tw_lower']+x['pu_stop_duration'], axis=1)
    ...
    payloads_FH['truck_mode']=B2BF_FH['mode_choice']
    payloads_FH['BuyerNAICS']=B2BF_FH['BuyerNAICS']
    payloads_FH['SellerNAICS']=B2BF_FH['SellerNAICS']
...
def sythfirm_fleet_file(fdir,year, scenario):
    ...
    return firm_file, warehouse_file, leasing_file, stock_file, port_file 
# %%
def main(args=None):
    # read input files
    ...
    firm_file, warehouse_file, leasing_file, stock_file, port_file =sythfirm_fleet_file(config.fdir_in_out,args.target_year, args.scenario)
    ...
    firms, truckings,leasings, dist_df, CBGzone_df, df_dpt_dist, ex_zone_list, ex_zone= genral_input_files_processing(firm_file, port_file,                                                        
                                                                                     warehouse_file,
                                                                                     leasing_file,
                                                                                     stock_file,
                                                                                     args.target_year,
 ...
 ###############
    # growth_factor=100
    # firm_file, warehouse_file, leasing_file, stock_file, port_file =sythfirm_fleet_file(config.fdir_in_out,2018, "Base")
    # create_global_variable(config.md_cap,config.hd_cap,config.fdir_in_out ,config.state_id,config.max_tour_for_b2b, 10)
    # # Read general files including geo data, firm and trukcing population data
    # print ("**** Processing general data sets ****")
    # firms, truckings,leasings, dist_df, CBGzone_df, df_dpt_dist, ex_zone_list, ex_zone= genral_input_files_processing(firm_file, port_file,
    #                                                                                  warehouse_file,
    ...
    # target_year= "2018"
    # df_hh_D, df_hh_groc, df_hh_food= b2c_input_files_processing(CBGzone_df, 
    #                                     config.b2c_delivery_frequency, sel_county, config.list_error_zone, growth_factor, target_year)
    ...
    # scenario= "Base"
    # daily_demand_creator= "Y"
    # df_vius= pd.read_csv(fdir_in_out+"/Model_carrier_op/VIUS/vehicle_proportion_by_sctg_dist.csv", header=0, sep=',')
    # FH_B2B, PV_B2B, firms = b2b_input_files_processing(firms,leasings,truckings,dist_df, CBGzone_df, sel_county, ship_direction, 
    # config.commodity_list, config.weight_theshold, config.list_error_zone,config.county_list,df_vius, config.b2b_day_factor, 
    # target_year, scenario, daily_demand_creator)
    # FH_B2B.to_csv(fdir_in_out+'/Sim_outputs/temp_save/FH_B2B_county%s_ship%s.csv' %(sel_county, ship_direction), index = False, header=True)
    # PV_B2B.to_csv(fdir_in_out+'/Sim_outputs/temp_save/PV_B2B_county%s_ship%s.csv' %(sel_county, ship_direction), index = False, header=True)
    # firms.to_csv(fdir_in_out+'/Sim_outputs/temp_save/B2B_firms%s_ship%s.csv' %(sel_county, ship_direction), index = False, header=True)
######################
...
############################################# on_demnad processing 
        if args.b2c_type in ["all", "grocery"]: 
            ...
            if df_hh_groc.shape[0]>0:
                df_input_org=df_hh_groc.sample(frac=sample_ratio/100).reset_index(drop=True)
            else:
                df_input_org=df_hh_groc
        ...
        if args.b2c_type in ["all", "food"]: 
            ...
            if df_hh_food.shape[0]>0:
                df_input_org=df_hh_food.sample(frac=sample_ratio/100).reset_index(drop=True)
            else:
                df_input_org=df_hh_food       
            ...
            for v_type in output_veh_list:
                ...
                if num ==0:
                    ...
                    if "md" in v_type:
                        ...
                        v_text="HD"        
                    temp_carr['depot_lower']= temp_carr.apply(lambda x: depot_time_depart_v2(df_dpt_dist,args.ship_type, v_text), axis=1)
                    temp_carr['depot_upper']= temp_carr['depot_lower'].apply(lambda x: 24*60 if x < 19*60 else x + 5*60)    
