...
def main(args=None):
    ...
    if args.ship_type == "B2C":
        ...
        # truckings_org= truckings.copy()
        # df_vius= pd.read_csv(fdir_in_out+"/Model_carrier_op/VIUS/vehicle_proportion_by_sctg_dist.csv", header=0, sep=',')
        # FH_B2B, PV_B2B, firms = b2b_input_files_processing(firms,leasings,truckings,dist_df, CBGzone_df, 1, "all", 
        # config.commodity_list, config.weight_theshold, config.list_error_zone,config.county_list,df_vius, config.b2b_day_factor, 
        # 2018, "Base")
        # FH_B2B.to_csv(fdir_in_out+'/Sim_outputs/temp_save/FH_B2B_county%s_ship%s.csv' %(1, "all"), index = False, header=True)
        # PV_B2B.to_csv(fdir_in_out+'/Sim_outputs/temp_save/PV_B2B_county%s_ship%s.csv' %(1, "all"), index = False, header=True)
        # firms.to_csv(fdir_in_out+'/Sim_outputs/temp_save/B2B_firms%s_ship%s.csv' %(1, "all"), index = False, header=True)        
        ...
        # PV_B2B['payload_id']=PV_B2B.index
        # FH_B2B['payload_id']=FH_B2B.index + PV_B2B.shape[0]
        # PV_B2B['payload_id']=PV_B2B['payload_id'].apply(lambda x: str(1) + '_' + "B2B" + str(int(x)))
        # FH_B2B['payload_id']=FH_B2B['payload_id'].apply(lambda x: str(1) + '_' + "B2B" + str(int(x)))
        # id_lookup=pd.concat([PV_B2B[['payload_id','shipment_id']],FH_B2B[['payload_id','shipment_id']]], ignore_index=True)        
        # Assing x_y
