...
def genral_input_files_processing(firm_file, warehouse_file, leasing_file, stock_file, target_year, scenario, dist_file,CBG_file, ship_type,list_error_zone, county_list):
    ...
    if ship_type == 'B2C':
        ...
        for veh_type in input_veh_list[:-1]:
            truckings[veh_type]=truckings[veh_type].apply(lambda x: x*1)
        truckings['md_capacity']=truckings.apply(lambda x: (x['Diesel Class 4-6 Vocational'] + x['Electric Class 4-6 Vocational'])*md_max_load, axis=1)
        truckings['hdt_capacity']=truckings.apply(lambda x: (x['Diesel Class 7&8 Tractor'] + x['Electric Class 7&8 Tractor']) *hd_max_load, axis=1)
        truckings['hdv_capacity']=truckings.apply(lambda x: (x['Diesel Class 7&8 Vocational'] + x['Electric Class 7&8 Vocational']) *hd_max_load, axis=1)
...
def b2b_veh_type_truckload_prior_old(SCTG_Group,Distance, D_truckload, df_vius,dic_firm_stock):
    ...
    if prob_veh < md_val/(hd_tractor_val+hd_voc_val+md_val): # MD
        ...
        hd_voc_load = 0    
    return [md_load, hd_voc_load, hd_trac_load, v_type, col_name]
def b2b_veh_type_truckload_old(SCTG_Group,Distance, D_truckload, df_vius):
    ...
    hd_tractor_val=df_vius[(df_vius["sctg"]==SCTG_Group) & (df_vius['VEH_CLASS_SynthFirm']=='HDT tractor')][col_name].values[0]
    hd_voc_val=df_vius[(df_vius["sctg"]==SCTG_Group) & (df_vius['VEH_CLASS_SynthFirm']=='HDT vocational')][col_name].values[0]
    md_val=df_vius[(df_vius["sctg"]==SCTG_Group) & (df_vius['VEH_CLASS_SynthFirm']=='MDT vocational')][col_name].values[0]
    prob_veh = random.uniform(0,1)
    if prob_veh <= md_val: # MD
        v_type ="md"
        md_load = D_truckload
        hd_trac_load = 0
        hd_voc_load = 0
    elif prob_veh > md_val and prob_veh <= (md_val+hd_voc_val): # hd_voc
        v_type ="hd_voc"
        md_load = 0
        hd_trac_load = 0
        hd_voc_load = D_truckload
...
def b2b_create_output(B2BF_PV,B2BF_FH,truckings,df_dpt_dist, ship_type, ex_zone_list, firms,ex_zone):
    ...
    payloads_FH['del_stop_duration']=B2BF_FH.apply(lambda x: int(np.random.gamma(2, 1, 1)[0]*((x['D_truckload'] -load_min)/(load_max-load_min))*60) if x['outbound_index'] ==0 else 60*10, axis=1)
...
#################################
#     growth_factor=100
#     firm_file, warehouse_file, leasing_file, stock_file =sythfirm_fleet_file(config.fdir_in_out,2018, "Base")
...
#                                         config.b2c_delivery_frequency, sel_county, config.list_error_zone, growth_factor, 2018)   
